...
const MyProjects = ({type, title, summary, img, link, github}) => {
    return(
        <motion.article className='relative w-full rounded-[1.2rem] rounded-br-2xl mt-[4rem]  shadow-2xl'
        initial={{y:50}}
        whileInView={{y:0}}
...
const Project = ({title, type, img, link, summary, github}) =>{
    return (
<motion.article className='relative w-full h-[130%]  rounded-[1.2rem] rounded-br-2xl   shadow-2xl'
initial={{y:50}}
whileInView={{y:0}}
transition={{duration:1.5, type: "spring"}}
>        
        ...
        <div className='w-full flex flex-col items-start justify-between pl-6'>
            ...
            <div className='mb-5 flex items-center'>
                ...
                <Link href={github} target='_blank'className='ml-2 underline font-semibold'>
                    visit
                </Link>
            </div>
